# Merge samples that were sequenced multiple times on different lanes
# (or within the same lane, with different barcodes)
...
fastq_list <-
  read_lines(paste0(basedir, "sample_lists/fastq_list.txt"))
fastq_table <-
  read_tsv(paste0(basedir, "sample_lists/fastq_table.txt"))
...
duplicated_samples <-
  (target_samples$sample_id)[duplicated(target_samples$sample_id)] %>% unique()

## Check if there are duplicated samples
if (length(duplicated_samples) == 0) {
  print("There are no samples to merge. Run generate_bam_list.sh")
}else{
  # Write cd
  ...
  for (i in 1:length(duplicated_samples)) { # nolint
    dup_sample_dat <-
      filter(fastq_table, sample_id == duplicated_samples[i])
    ...
    output <-
      paste(
        dup_sample_dat[1, "sample_id"],
        dup_sample_dat[1, "population"],
        "merged_bt2",
        refname,
        "minq20_sorted.bam",
        sep = "_"
    ...
    if (i == 1) {
      write_lines(
        paste(dup_sample_dat[1, "sample_id"], dup_sample_dat[1, "population"], "merged", sep = "_"),
        paste0(basedir, "sample_lists/merged_bam_list.txt")
      )
    }
